...
SERIES_LABELS = {
    ...
    'mean_core_warning_f1': 'Mean Core Warning F1',
    'mean_core_warning_recall': 'Mean Core Warning Recall',
}
...
SERIES_ORDER = {'warning_f1': 0, 'mean_core_warning_f1': 0, 'warning_recall': 1, 'mean_core_warning_recall': 1}
...
def _series_colors(series_names: list[str]) -> list[str]:
    ...
    preferred = {
        'warning_f1': palette['teal'],
        'warning_recall': palette['coral'],
        'mean_core_warning_f1': palette['teal'],
        'mean_core_warning_recall': palette['coral'],
    }
    fallback = [palette['teal'], palette['coral'], palette['sage'], palette['gold']]
    return [preferred.get(name, fallback[idx % len(fallback)]) for idx, name in enumerate(series_names)]
...
def main() -> None:
    ...
    parser.add_argument('--title', default='Core warning performance')
    ...
    ax.set_ylim(0.0, min(1.02, max(0.82, float(pivot.to_numpy(dtype=float).max()) + 0.12)))
